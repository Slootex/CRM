<div class=" w-full px-6 py-6">
  ...
  ```
-->
<form action="{{url('/')}}/crm/auftragshistory/new/{{$person->process_id}}" class="relative" method="POST">
    @CSRF
<div>
...
<div class="flex items-start space-x-4">
    ...
    <div class="min-w-0 flex-1 ">
        ...
        <div class="overflow-hidden rounded-lg border border-gray-300 shadow-sm focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
          ...
          <textarea rows="3" name="comment" id="comment" class="block w-full resize-none border-0 py-3 focus:ring-0 sm:text-sm" placeholder="Kommentar"></textarea>
  ...
  <table class="min-w-full divide-y divide-gray-300">
      <thead class="bg-gray-50">
        <tr>
          <th scope="col" class="py-1 pl-1 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Datum</th>
      ...
      <tbody class="bg-white">
        @isset($auftragshistory)
        @foreach ($auftragshistory as $af)
        <tr>
          <td class="whitespace-nowrap px-1 py-1 text-sm text-left text-gray-500">{{$af->created_at->format("d.m.Y")}} ({{$af->created_at->format("H:i")}})</td> 
          <td class="inline-flex rounded-full bg-green-100 px-1 text-left text-xs font-semibold leading-5">Auftragsnachricht</td>
          <td class="whitespace-nowrap px-1 py-1 text-sm text-left text-gray-500">{{$af->employee}}</td>
          <td class="whitespace-nowrap px-1 py-1 text-sm  text-left text-gray-500">{{$af->message}}</td>
          <td class="whitespace-nowrap px-1 py-1 text-sm  text-left text-blue-500"><svg onclick="readStatus('{{$af->process_id}}','{{$af->created_at}}')" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    @endforeach
        @endisset
        @foreach ($status_history->where("area", "packtisch") as $st)

            <tr>
                <td class="whitespace-nowrap px-1 py-1 text-sm text-left text-gray-500">{{$st->created_at->format("d.m.Y")}} ({{$st->created_at->format("H:i")}})</td> 
                <td class="whitespace-nowrap px-1 py-1 text-sm text-left text-gray-500">{{$st->employee}}</td>
                @foreach ($statuses as $stat)
                @if ($stat->id == $st->last_status)
                ...
                @endif
            @endforeach
                 <td class="whitespace-nowrap px-1 py-1 text-sm  text-left text-gray-500"><a class="text-blue-500 underline" href="{{url('/')}}/files/aufträge/{{$st->process_id}}/{{$st->component_number}}_aktuell_.pdf"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 float-left mr-2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M18.375 12.739l-7.693 7.693a4.5 4.5 0 01-6.364-6.364l10.94-10.94A3 3 0 1119.5 7.372L8.552 18.32m.009-.01l-.01.01m5.699-9.941l-7.81 7.81a1.5 1.5 0 002.112 2.13" />
                </svg>
                </a>, Lager: {{$st->old_shelfe}}@isset($st->bpz1), BPZ-1: {{$st->bpz1}} @endisset @isset($st->bpz2), BPZ-2: {{$st->bpz2}} @endisset @isset($st->label), Label: {{$st->label}} @endisset @isset($st->opend), Geöffnet: {{$st->opend}} @endisset @isset($st->sticker), Andere Sticker: {{$st->sticker}} @endisset</td>
                <td class="whitespace-nowrap px-1 py-1 text-sm  text-left text-blue-500"><svg onclick="readStatus('{{$st->process_id}}','{{$st->created_at}}')" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                </svg>
                </td>

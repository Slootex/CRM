    ...
    <div class="fixed m-auto inset-0 z-50 overflow-y-auto " >
      <div class="flex min-h-full items-end justify-center p-8 text-center sm:items-center sm:p-0" >
        <div class="relative transform overflow-hidden rounded-lg bg-white px-8 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8  sm:p-6" style="width: 80rem;">
          <form action="{{url("/")}}/crm/buchhaltung/edit-rechnung" method="POST" id="edit-rechnung-form">
            @csrf
           ...
           <div class="px-6 pt-0 w-full">
            ...
            <div class="float-right">
              <button type="button" onclick="document.getElementById('edit-rechnung-modal').innerHTML = '';" class="float-right">
              ...
              <div class="pb-4">
                <div class="float-left flex">
                    <select id="neue-rechnung-modal-select-type" name="type" onchange="changeNeueRechnungType(this.value)" class="block w-36 h-10 rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-blue-600 sm:text-sm sm:leading-6">
                        <option value="Rechnung" @if(substr($rechnungen[0]->rechnungsnummer, 0, 1) == "F") selected @endif>Rechnung</option>
                        <option value="Angebot" @if(substr($rechnungen[0]->rechnungsnummer, 0, 1) == "A") selected @endif>Angebot</option>
                        <option value="Gutschrift" @if(substr($rechnungen[0]->rechnungsnummer, 0, 1) == "G") selected @endif>Gutschrift</option>
                    </select>
                    <div class="relative ml-4 @if(substr($rechnungen[0]->rechnungsnummer, 0, 1) == "G") hidden @endif" id="zahlungsziel-neue-rechnung">
                        ...
                        <input type="text" required value="{{$rechnungen[0]->zahlungsziel}}" name="zahlungsziel" id="zahlungsziel" class="block w-24 h-10 rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
                    </div>
                    <div class="relative ml-4 @if(substr($rechnungen[0]->rechnungsnummer, 0, 1) != "G") hidden @endif" id="vergleich-neue-rechnung">
                ...
                <div onclick="toggleMWST()" class="flex cursor-pointer float-right mr-4 rounded-md border border-gray-300 hover:bg-blue-100 px-4 h-10">
                    <div class="flex mt-2.5">
                      ...
                      <button type="button" id="mwst-toggle-button" class="ml-2 group relative inline-flex h-5 w-10 flex-shrink-0 items-center justify-center rounded-full focus:outline-none" role="switch"  @if($rechnungen[0]->mwst == 'no') aria-checked="false" @else  aria-checked="true" @endif>
                          <span @if($rechnungen[0]->mwst == 'no') aria-checked="false" @else  aria-checked="true" @endif class="pointer-events-none absolute h-full w-full rounded-md bg-white"></span>
                          <span @if($rechnungen[0]->mwst == 'no') aria-checked="false" @else  aria-checked="true" @endif id="mwst-toggle-button-color" class="@if($rechnungen[0]->mwst == 'no') bg-gray-200 @else bg-blue-600 @endif pointer-events-none absolute mx-auto h-4 w-9 rounded-full transition-colors duration-200 ease-in-out"></span>
                          <span @if($rechnungen[0]->mwst == 'no') aria-checked="false" @else  aria-checked="true" @endif id="mwst-toggle-button-translate" class="@if($rechnungen[0]->mwst == 'no') translate-x-0 @else translate-x-5 @endif pointer-events-none absolute left-0 inline-block h-5 w-5 transform rounded-full border border-gray-200 bg-white shadow ring-0 transition-transform duration-200 ease-in-out"></span>
                      </button>
                    </div>
                      ...
                      <table class="min-w-full divide-y divide-gray-300">
                        <thead>
                          <tr>
                            ...
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 @if($rechnungen[0]->mwst == 'no') hidden @endif" id="neue-rechnung-table-mwst">MwSt</th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Netto Betrag</th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 @if($rechnungen[0]->mwst == 'no') hidden @endif" id="neue-rechnung-table-mwstbetrag">MwSt Betrag</th>
                        ...
                        <tbody class="divide-y divide-gray-200" id="neue-rechnung-positions">
                          @include('includes.rechnungen.neue-rechnung-positions-table')
                        </tbody>
                      </table>
                    </div>
                  </div>
                        ...
                        @else
                            <div class="py-2 px-4 rounded-md bg-yellow-100 float-left">
                                <div class="flex text-yellow-700 text-sm">
                                    ...
                                    <p class="text-black ml-2 truncate" style="max-width: 8rem;" id="rechnungstext-name">{{$rechnungstexte->where("id", $rechnungen[0]->rechnungstext)->first()->title}}</p>
                            ...
                            <div class="ml-4 hidden" id="rechnungstext-select-div">
                                <select onchange="document.getElementById('rechnungstext-name').innerHTML = this.value; document,getElementById('rechnungstext-select-div').classList.add('hidden')" name="rechnungstext" class="w-60 rounded-md border border-gray-300 h-9" style="padding: 5px">
                                    @foreach ($rechnungstexte as $text)
                                        <option value="{{$text->title}}" @if($text->id == $rechnungen[0]->rechnungstext) selected @endif >{{$text->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endif
...
<div class="flex min-h-full items-end justify-center p-8 text-center sm:items-center sm:p-0" >
  <div class="relative transform overflow-hidden rounded-md bg-white px-8 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8  sm:p-6" style="width: 20rem;">
    <form action="{{url("/")}}/crm/buchhaltung/rechnung/neue-position/{{$kundenkonto->process_id}}" method="POST" onsubmit="loadData(); document.getElementById('rechnung-neue-position-modal').classList.add('hidden');" id="neue-position-form">
      @CSRF
      <input type="hidden" name="temp_id" value="{{$rechnungen[0]->rechnungsnummer}}" id="neue-position-temp-id">

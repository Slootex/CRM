...
@livewireScripts
<body>
@include('layouts/top-menu', ["menu" => "auftrag", "undermenu" => "archiv"])
@php
    ...
    $offeneRechnungen = 0;
@endphp
@foreach ($active_orders as $orders)
    @php
         $offen = 0;
         $zahlt = 0;
     @endphp
     @foreach ($orders->rechnungen as $rechnung)
        ...
        @if (!in_array($rechnung->rechnungsnummer, $usedRechnungen))
          ...
          @foreach ($rechnung->zahlungen as $zahlung)
              @php
                  ...
                  $zahlt += $zahlung->betrag;
              @endphp
          @endforeach
        @endif
      @php
        $zuzahlen += $rechnung->bruttobetrag;
        $offen += $rechnung->bruttobetrag;
      @endphp
     @endforeach
    @if ($zahlt <= $offen)
  ...
  <div class="px-8">
    ...
    <div class="pt-5">
     ...
     <div class="h-16 w-full">
       <h1 class="text-4xl font-bold pt-2 pb-3 text-black float-left">Auftragsarchiv @isset($filter)- Filter: {{ucfirst($filter)}} @endisset @isset($filterKunde)- Filter: K{{$filterKunde}}, Kunde: {{$active_orders->where("kunden_id", $filterKunde)->first()->firstname}} {{$active_orders->where("kunden_id", $filterKunde)->first()->lastname}} @endisset</h1>
         ...
         <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
           <table class="w-full divide-y divide-gray-300">
             <thead class="bg-gray-50">
               ...
               @foreach ($hilfscodes->where("archiv", true) as $code)
               <tr class="bg-red-200 border-l-0 border-r-0">
                 ...
                 <td class="whitespace-nowrap text-right py-1 text-sm text-black ">
                  <a class="text-blue-600 hover:text-blue-400 pr-2" href="{{url("/")}}/crm/wareneingang-archiv-zuweisen-{{$code->process_id}}">
                    Auftrag zuweisen
                   </a>  
                 </td>
                    ...
                    @if(isset($sorting))
                      ...
                      @foreach ($active_orders as $order)
                        ...
                        <tr class="hover:bg-blue-100 border-l-0 border-r-0" id="row-{{$orderCounter}}" onclick="selectQuickStatus('{{$orderCounter}}', 'no', event)">
                          ...
                          <td class="whitespace-nowrap  py-1 text-sm text-black" >
                           <div>
                            @if ($statuses->where("id", $order->statuse->sortByDesc("created_at")->first()->last_status)->first() != null)

                            <div style="background-color: {{$statuses->where("id", $order->statuse->sortByDesc("created_at")->first()->last_status)->first()->color}}" class="px-2 float-left py-0.5 text-sm rounded-xl text-center font-medium">

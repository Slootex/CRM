...
<body>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-10">
        <h1 class="text-3xl font-bold text-white mb-4">Gerät zu anderen Auftrag zuweisen</h1>
        <div class="overflow-hidden bg-white shadow sm:rounded-lg" style="height: 43rem">
            <div class="px-4 py-5 sm:p-6">

              <div class="float-left" style="width: 34.2rem;">
                <div>
                ...
                <div>
                  ...
                  <button type="button" onclick="searchOrderNumber()" class="bg-blue-600 hover:bg-blue-500 rounded-md font-semibold text-white text-xl px-4 py-2 ml-16">Suchen</button>
                </div>
                ...
                <div class="mt-10 mb-10 hidden float-left" id="used-device-div">
                  ...
                  <div class="w-60 h-28 rounded-lg shadow-md border border-gray-300 mt-4 px-2 py-2">
                    <h2 class="text-2xl font-semibold">neuer Gerätename</h2>
                ...
                <button type="button" onclick="document.getElementById('final-req-modal').classList.remove('hidden')" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-md text-white font-semibold hidden mt-4" id="finish-button">An Packtisch senden</button>
              </div>

              <div class="float-right" style="width: 34.2rem;">
                <div>
                  <h1 class="text-2xl font-bold mb-2">Dokumente zum Gerät</h1>
                </div>
      ...
      <script>
        ...
        function addOrderHelpercode(id) {
          loadData();
          if(document.getElementById('org-'+id).checked || document.getElementById('at-'+id).checked) {
            if(document.getElementById('org-'+id).checked) {
...
<form action="{{url("/")}}/crm/gerät-neu-zuweisen" method="POST">
...
<div class="relative hidden z-10" id="final-req-modal" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
      ...
      <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
        <div class="sm:flex sm:items-start">
          ...
          <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
            <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">Zuweisung bearbeiten</h3>
            <div class="mt-2">
              <p class="text-sm text-gray-500">Bei Änderung der Zuweisung wird ein Beschriftungsauftrag ausgelöst?</p>
            </div>
          </div>
        </div>
        <div class="mt-5 sm:ml-10 sm:mt-4 sm:flex sm:pl-4">
          <button type="button" onclick="sendRequest()" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:w-auto">Änderungen speichern</button>
          <button type="button" onclick="document.getElementById('final-req-modal').classList.add('hidden')" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:ml-3 sm:mt-0 sm:w-auto">Abbrechen</button>
        </div>
      </div>
    </div>
  </div>

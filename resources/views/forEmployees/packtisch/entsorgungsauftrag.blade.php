        ...
        <div class="px-4 py-5 sm:p-6">
            ...
            <div class="float-right w-60 bg-blue-200 rounded overflow-auto" style="height: 21rem; margin-top: -8.5rem" id="shelfes">
                ...
                @php
                    $shelfestate = false;
                @endphp
                    <p class="mt-4 ml-2" id="shelfe-{{$device->process_id."-".$device->component_id."-".$device->component_type."-".$device->component_count}}">{{$device->process_id."-".$device->component_id."-".$device->component_type."-".$device->component_count}}, @foreach ($shelfes as $shelfe)
                        @if ($device->process_id."-".$device->component_id."-".$device->component_type."-".$device->component_count == $shelfe->component_number)
                            {{$shelfe->shelfe_id}}
                            @php
                                $shelfestate = true;
                            @endphp
      ...
      <script>
        ...
        function addBarcode(e) {
            ...
            if(barcodestate == true && shelfecode == true) {
                ...
                document.getElementById("shelfe-" + codes).remove();

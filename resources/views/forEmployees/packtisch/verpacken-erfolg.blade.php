...
<body onload="printJS('label_frame', 'html')">
    ...
    <div style="width: 40%;" class="m-auto pb-60 rounded-md bg-white p-4 mt-10">
      ...
      <div class="mt-4">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-36 text-green-400 h-36 m-auto">
      ...
      <div>
        <img src="{{url("/")}}/temp/{{$number}}.png" class="w-full  mt-48" style="transform: rotate(90deg)" frameborder="0">
      </div>

      <div class="w-32 m-auto">
        <button type="button" onclick="printLabel('entsorgung')" class="text-white text-xl font-semibold px-6 py-3 rounded-md m-auto mt-16 bg-blue-600 hover:bg-blue-500 absolute">Drucken</button>
      </div>
...
<script>
  function checkLabel(e) {
    if(e.keyCode === 13){
      if(document.getElementById("label").value == "{{$number}}") {

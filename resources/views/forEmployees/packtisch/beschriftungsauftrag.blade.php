...
<body onload="test(); document.getElementById('barcode-input').focus()">
    ...
    <div class="mt-12" style="margin-bottom: -0.35rem">
      @include('includes.packtisch.scan-history')

    </div>
    <div>
      <form action="{{url("/")}}/crm/packtisch/beschriftungsauftrag-abschließen/{{$intern->id}}" method="POST">
        @csrf
        ...
        <div style="width: 40%" class=" bg-white rounded-md m-auto mt-6 pb-16">
          ...
          @endif
            ...
            <div class="inline-block px-8 py-4 w-full">
                <div style="width: 100%">
                    <div class="w-full">
                        <h1 class="font-bold inline-block text-3xl text-blue-800 mb-5">Beschriftierung</h1>
                        <button type="button" onclick="document.getElementById('problem-melden-modal').classList.remove('hidden'); document.getElementById('barcode-input').focus()" class="float-right  mt-1 bg-red-400 rounded-md text-white font-semibold px-3 py-1">Problem melden</button>
                      </div>
                    <div class="w-full">
                        ...
                        <ul role="list" class="-mb-8">
                              ...
                              <div class="relative flex space-x-3">
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div>
                                    ...
                                    <p class="text-xl text-gray-500">Aktuelle Gerätenummer <span class="font-semibold text-green-600">{{$shelfe->component_number}}</span></p>
                          ...
                          <li class="hidden" id="step-2">
                            <div class="relative pb-8">
                              ...
                              <div class="relative flex space-x-3">
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div>
                                    <button type="button" onclick="printJS('printJS-form', 'html');  document.getElementById('step-3').classList.remove('hidden'); document.getElementById('lagerplatz-input').focus()">
                          ...
                          <li class="hidden" id="step-3">
                            <div class="relative pb-8">
                              ...
                              <div class="relative flex space-x-3">
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div>
                                    <p class="text-xl text-gray-500">Neuer Barcode <span class="font-semibold text-green-600">{{$newBarcode}}</span></p>
                                    <input type="text" name="newBarcode" id="lagerplatz-input" onkeydown="checkNewBarcode(event, this.value)" class=" rounded-lg w-60 h-12 text-center border border-gray-600 text-xl">
                                  </div>
                          ...
                          <li class="hidden" id="step-4">
                            <div class="relative ">
                              ...
                              <div class="relative flex space-x-3">
                                <div>
                                  <span class="h-14 w-14 rounded-full flex items-center justify-center ring-8 ring-white">
                                    <svg id="devicedokumente-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-14 h-14 text-red-600">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                      </svg>
                                      <svg xmlns="http://www.w3.org/2000/svg" id="devicedokumente-svg-not" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-11 h-11 text-white bg-red-600 rounded-full hidden">
                          ...
                          <li class="hidden mt-8" id="step-5">
                            <div class="relative pb-8">
                              <span class="absolute left-7 top-7 -ml-px h-full w-0.5 bg-gray-200" aria-hidden="true"></span>
                              <div class="relative flex space-x-3">
                                <div>
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div>
                                    ...
                                    <input type="text" name="shelfe" id="neuer-lagerplatz-input" onkeypress="checkShelfe(event, this.value)" class=" rounded-lg w-60 h-12 text-center border border-gray-600 text-xl">
                                  </div>
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div class="m-auto w-full">
                                    <button type="submit" id="einlagern-button" class="text-white inline-block font-semibold py-3 rounded-md bg-blue-600 hover:bg-blue-500 text-xl" style="width: 35rem">
                                        ...
                                        <p class="inline-block" id="submit-type-text">Fertig, beschriftet</p>
                                      </button>
                                  </div>
      ...
      <script>
          ...
          if(e.keyCode == 13) {
            e.preventDefault();
              if(input != '') { 
                  ...
                  scanFound(input, "Beschriftungsauftrag");
                  ...
                  scanNotFound(input, "Beschriftungsauftrag");
                }
              }
            }
          }
        ...
        function lookupDevicesDocs() {
          ...
          function checkNewBarcode(e, input) {
        ...
        function saveOrderDocuments(device) {
            $.get("{{url('/')}}/crm/packtisch/get-devicedokumente-documents/"+device, function(data){
                ...
                document.getElementById("step-4").classList.remove("hidden");
                document.getElementById('neuer-lagerplatz-input').focus()
...
JsBarcode(element, "{{$intern->process_id}}", {
  height: 60,
  fontSize: 20,
  width: 2.5,
  textMargin: -2,
  text: "{{$intern->process_id}}"

...
<body onload="test()">
      ...
      <form action="{{url("/")}}/crm/packtisch/neues-gerät-einlagern/{{$kunde->process_id}}" method="POST">
        ...
        <div style="width: 40%" class=" bg-white rounded-md m-auto mt-6 pb-16">
            <div class="inline-block px-8 py-4 w-full">
                <div style="width: 100%">
                    <div class="w-full">
                        <h1 class="font-bold inline-block text-3xl text-blue-800 mb-5">Neues Gerät einlagern</h1>
                              ...
                              <div class="relative flex space-x-3">
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div>
                                    ...
                                    @isset($device->component_number)
                                        <p class="text-xl text-gray-500">Neue Gerätnummer <span class="text-blue-600 font-medium">{{$device->process_id}}</span></p>
                                        @else
                                        @if (!is_array($device))
                                        <p class="text-xl text-gray-500">Neue Gerätnummer <span class="text-blue-600 font-medium">{{$device}}</span></p>
                                        @else 
                                        <p class="text-xl text-gray-500">Neue Gerätnummer <span class="text-blue-600 font-medium">{{$kunde->process_id."-".$device[0]."-".$device[1]."-".$device[2]}}</span></p>
                                        @endif
                                    @endisset
                                ...
                                <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                  <div class="m-auto flex w-full">
                                    <button type="submit" id="einlagern-button" class="text-white font-semibold py-3 rounded-l-md rounded-r-none bg-blue-600 hover:bg-blue-500 text-xl" style="width: 35rem">
                                        ...
                                        <p class="inline-block" id="submit-type-text">Neues Gerät einlagern</p>
    ...
    <script>
        ...
        function lookupOrderDocs() {
          ...
          iframe.src = "{{url("/")}}/files/aufträge/{{$kunde->process_id}}/{{$kunde->process_id."-".$device[0]."-".$device[1]."-".$device[2]}}-a.pdf";
        ...
        function lookupDevicesDocs() {
          ...
          let iframe = document.createElement("iframe");
          iframe.src = "{{url("/")}}/files/aufträge/{{$kunde->process_id}}/{{$kunde->process_id."-".$device[0]."-".$device[1]."-".$device[2]}}-g.pdf";
          iframe.classList.add("w-full", "h-full", "mt-6");
          iframe.style.height = "45rem";

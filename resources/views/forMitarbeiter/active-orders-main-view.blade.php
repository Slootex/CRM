<html lang="de"><head><title>CRM P+</title>
    ...
    <div class="row">
        <div class="col-lg-7 col-md-7 col-sm-7 col-xs-7">
            <ul class="nav nav-pills">
                ...
                <li class="nav-item">
                    <a href="{{url("/")}}/crm/neuer-auftrag" class="nav-link">Neuer Auftrag</a>
                </li>
                <li class="nav-item">
        ...
        <div class="col-sm-5 text-right">
            <form action="/crm/orders/search/" method="post">
                @CSRF
    ...
    <div class="col-sm-7 text-right"><input type="hidden" name="admin_id" value="31">
    ...
    <div class="card_lb_success bg-white text-left shadow">
        <div class="card_lb_content">
            <span class="card_lb_title text-success">Neuaufträge</span><span class="card_lb_percent">7%</span><br>
            <span class="card_lb_value">4 von 53</span>
        </div>
    ...
    <div class="card_lb_danger bg-white text-left shadow">
        <div class="card_lb_content">
            <span class="card_lb_title text-danger">Prüfungen</span><span class="card_lb_percent">3%</span><br>
            <span class="card_lb_value">2 von 53</span>
        </div>
        <div class="card_lb_logo text-center mt-1">
    ...
    <tr><td>25.10.2022</td><td class='text-center'>3</td><td class='text-center'>0</td><td class='text-center'>3</td></tr>
    </table>">3</a></span>
        </div>
        <div class="card_lb_logo text-center mt-1">
    ...
    <div class="row">
        ...
        <div class="col-lg-5 col-md-5 col-sm-5 col-xs-5 text-right">
            <form id="order_search" action="/crm/orders/search/" method="post">
                {{ csrf_field() }}
                @CSRF
                <div class="form-group row mb-1">
                    <div class="col-sm-12">
                        ...
                        <div class="my-dropdown-menu bg-white rounded-bottom border border-primary p-3 mr-3" style="position: absolute;top: 40px;right: 0px;margin-bottom: 30px;width: 200px;display: none;box-shadow: 0 0 4px #000;z-Index: 1000">
                            <h6 class="text-left">Einträge&nbsp;pro&nbsp;Seite</h6>
                            <select id="rows" name="rows" class="custom-select text-primary border border-primary" onchange="">
                                <option value="10">10</option>
                                <option value="20">20</option>
                                <option value="40">40</option>
                                <option value="50">50</option>
                            ...
                            <h6 class="text-left mt-2">Sortierfeld</h6>
                            <select id="sorting_field" name="sorting_field" class="custom-select text-primary border border-primary" onchange="">
    ...
    <option value="4">Auftragsnummer</option>
                            ...
                            <h6 class="text-left mt-2">Sortierrichtung</h6>
                            <select id="sorting_direction" name="sorting_direction" class="custom-select text-primary border border-primary" onchange="">
    ...
    <form action="/crm/neue-auftraege" method="post"></form>
        <tbody>
        @foreach ($active_orders as $order)
                
            <tr class="orders_menu" onclick="if($(this).hasClass('active')){$(this).removeClass('active');}else{$(this).addClass('active');}$('#order_list_5095').prop('checked', !$('#order_list_5095').prop('checked'))" data-id="5095" data-order_number="25217">
            <td scope="row">
                <div class="custom-control custom-checkbox mt-1 ml-2">
                    <input type="checkbox" id="order_list_5095" data-id="5095" class="custom-control-input order-list" onclick="if($(this).closest('tr').hasClass('active')){$(this).closest('tr').removeClass('active');}else{$(this).closest('tr').addClass('active');}">
                    <label class="custom-control-label" for="order_list_5095"></label>
                </div>
            </td>
            ...
            <td class="text-nowrap">
                <small>{{$order->created_at->format("d.m.Y")}}</small> <small class="text-muted">({{$order->created_at->format("h:m")}})</small>
            </td>
            <td scope="row">
            ...
            <td>
                <div style="width: 250px;white-space: nowrap;overflow-x: hidden"><small>{{$order->firstname}} {{$order->lastname}}</small></div>
            </td>
            <td align="center">
                @foreach ($order_ids as $order_id)
                    @if ($order_id->process_id == $order->process_id)
                        @foreach ($statuses as $status)
                            ...
                            <span class="badge badge-pill" style="background-color: {{$status->color}}">
                                {{$status->name}}
                            </span>
                            @endif
                        @endforeach
            ...
            <td class="text-nowrap">
                <small>{{$order->updated_at->format("d.m.Y")}} ({{$order->updated_at->format("h:m")}})</small>
            </td>
            <td>
                <div style="width: 120px;white-space: nowrap;overflow-x: hidden"><small>@foreach ($employees as $employee)
            ...
            <td>
                <small>{{$order->zuteilung}}</small>
            </td>
            <td>
                <small>{{$order->phone_number}}</small>
            </td>
            <td>
                <div style="width: 140px;white-space: nowrap;overflow-x: hidden"><small>{{$order->last_payment}}</small></div>
            </td>
            <td align="center">
                <div style="white-space: nowrap">
                    <input type="hidden" name="id" value="5095">
                    <button type="submit" name="edit" value="bearbeiten" class="btn btn-sm btn-success" onclick="window.location.href = '{{url('/')}}/crm/orders/change/{{$order->process_id}}'">bearbeiten <i class="fa fa-pencil" aria-hidden="true"></i></button>
                </div>
            </td>
        </tr>
        @endforeach
    ...
    <form action="/crm/neue-auftraege" method="post">
        <table class="table table-white table-sm mb-0">
            <tbody><tr class="text-primary">
                ...
                <td width="350">
                    <label for="order_sel_all_bottom" class="mt-1">alle auswählen (1 bis 53 von 53 Einträgen)</label>
                </td>
                <td width="260">
    ...
    var dates = [
        ...
        {"id": 5191, "mode": 2, "today": false, "date": "27.10.2022 10:15"}, 

...
$order_session = "order_orders_old";
$order_action = $page['url']; // "/crm/alte-auftraege";
$order_table = "order_orders";
$order_mode = 0;
$order_archiv_mode = 1;
$order_right = "order_orders_old";
$order_name = "Auftrag";
...
if(isset($_POST['id']) && $_POST['id'] > 0){
	...
	$row_order = mysqli_fetch_array(mysqli_query($con2, "	SELECT 		`wnm_csv`.`id` AS id, 
																		`wnm_csv`.`rma` AS rma, 
																		`wnm_csv`.`ext_rma` AS ext_rma, 
																		`wnm_csv`.`rgnr` AS rgnr, 
																		`wnm_csv`.`kdnr` AS kdnr, 
																		`wnm_csv`.`companyname` AS companyname, 
																		...
																		`wnm_csv`.`phonenumber` AS phonenumber, 
																		`wnm_csv`.`email` AS email, 
																		`wnm_csv`.`reg_date` AS reg_date 
															FROM 		`wnm_csv` 
															WHERE 		`wnm_csv`.`id`='" . intval($_POST['id']) . "'"), MYSQLI_ASSOC);
...
$where = 	isset($_SESSION[$order_session]["keyword"]) && $_SESSION[$order_session]["keyword"] != "" ? 
			"WHERE 	(`wnm_csv`.`rma` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`rgnr` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`ext_rma` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`carrier_tracking_no` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`kdnr` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`companyname` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`firstname` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`lastname` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`street` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`zipcode` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`city` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`country` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`phonenumber` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%' 
			OR		`wnm_csv`.`email` LIKE '%" . mysqli_real_escape_string($conn, $_SESSION[$order_session]["keyword"]) . "%') " : 
			"";

$and = $where == "" ? "WHERE 		`wnm_csv`.`id`>0 " : " AND 		`wnm_csv`.`id`>0 ";

$query = 	"	SELECT 		`wnm_csv`.`id` AS id, 
							`wnm_csv`.`m_key` AS m_key, 
							`wnm_csv`.`wnm_produkt` AS wnm_produkt, 
							`wnm_csv`.`rma` AS rma, 
							`wnm_csv`.`rgnr` AS rgnr, 
							`wnm_csv`.`ext_rma` AS ext_rma, 
							`wnm_csv`.`kdnr` AS kdnr, 
							...
							`wnm_csv`.`phonenumber` AS phonenumber, 
							`wnm_csv`.`email` AS email, 
							`wnm_csv`.`carrier_tracking_no` AS carrier_tracking_no, 
							`wnm_csv`.`end_sum` AS end_sum, 
							`wnm_csv`.`reg_date` AS reg_date 
				FROM 		`wnm_csv` 
				" . $where . $and . "";
...
$navigation = new navigation($conn, 3, "order_orders_old");

...
include("includes/class_payings.php");
...
$parameter['tab'] = "edit";
$parameter['link'] = "auftrag-archiv";

$order_session = "orders";
$order_action = "/crm/auftrag-archiv";
$order_table = "order_orders";
$order_id_field = "order_id";
$order_mode = 0;
$order_archiv_mode = 1;
$order_hash = "";
$order_templates_type = 1;
$order_right = "order_archive";
$order_name = "Archiv-Auftrag";
...
$sorting[] = array(
	"name" => "Übertragen", 
	"value" => "CAST(`order_orders`.`transfer_date` AS UNSIGNED)"
);
$sorting[] = array(
	"name" => "Erstelldatum", 
...
$sorting_field_name = isset($_SESSION[$order_session]["sorting_field"]) ? $sorting[$_SESSION[$order_session]["sorting_field"]]["value"] : $sorting[0]["value"];
$sorting_field_value = isset($_SESSION[$order_session]["sorting_field"]) ? $_SESSION[$order_session]["sorting_field"] : 0;
...
$sorting_direction_name = isset($_SESSION[$order_session]["sorting_direction"]) ? $directions[$_SESSION[$order_session]["sorting_direction"]] : "ASC";
$sorting_direction_value = isset($_SESSION[$order_session]["sorting_direction"]) ? $_SESSION[$order_session]["sorting_direction"] : 0;
...
if(isset($_POST['paying_new']) && $_POST['paying_new'] == "neu"){

	$parameter['tab'] = "intern";
	$parameter['link'] = "auftrag-archiv";

	include("includes/condition/admin_order_new_paying_neu.php");

}

if(isset($_POST['shopping_new']) && $_POST['shopping_new'] == "neu"){

	$parameter['tab'] = "intern";
	$parameter['link'] = "auftrag-archiv";

	include("includes/condition/admin_order_new_shopping_neu.php");
...
if(isset($_POST['id']) && $_POST['id'] > 0){
	...
	$payings = new payings($conn, $row_order['id']);
...
if(isset($_POST['move']) && $_POST['move'] == "Zu Aufträge"){
	...
	$parameter['archive_to_order_status'] = "archive_to_order_status";

	include("includes/condition/admin_order_move_zu_auftraege.php");

}

if(isset($_POST['move_interested']) && $_POST['move_interested'] == "Zu Interessenten"){

	$parameter['link'] = "auftrag-archiv";

	$parameter['archive_to_interested_status'] = "order_archive_to_interested_status";

	include("includes/condition/admin_order_move_interested_zu_interessenten.php");
...
$query = 	"	SELECT 		`order_orders`.`id` AS id, 
							...
							`order_orders`.`transfer_date` AS transfer_date, 
	...
	if($rows > 0){
		...
		while($row_orders = $result->fetch_array(MYSQLI_ASSOC)){
			...
			$list .= 	"<form action=\"" . $order_action . "\" method=\"post\">\n" . 
						"	<tr" . (isset($_POST['id']) && $_POST['id'] == $row_orders['id'] ? " class=\"bg-primary text-white orders_archiv_menu\"" : " class=\"orders_archiv_menu\"") . " onclick=\"if(\$(this).hasClass('active')){\$(this).removeClass('active');}else{\$(this).addClass('active');}$('#order_list_" . $row_orders['id'] . "').prop('checked', !$('#order_list_" . $row_orders['id'] . "').prop('checked'))\" data-id=\"" . $row_orders['id'] . "\" data-order_number=\"" . $row_orders['order_number'] . "\">\n" . 
						"		<td scope=\"row\">\n" . 
						...
						"			<small>" . date("d.m.Y", intval($row_orders['reg_date'])) . "</small> <small class=\"text-muted\">" . date("(H:i)", intval($row_orders['reg_date'])) . "</small>\n" . 
						"		</td>\n" . */
						"		<td class=\"text-nowrap\">\n" . 
						"			<small>" . date("d.m.Y", intval($row_orders['transfer_date'])) . "</small> <small class=\"text-muted\">" . date("(H:i)", intval($row_orders['transfer_date'])) . "</small>\n" . 
						"		</td>\n" . 
						"		<td scope=\"row\">\n" . 
						...
						"		</td>\n" . 
						"		<td>\n" . 
						"			<small>" . $row_orders['address_shortcut'] . "</small>\n" . 
						"		</td>\n" . 
						"		<td>\n" . 
						"			<small>" . ($row_orders['mobilnumber'] != "" ? $row_orders['mobilnumber'] : $row_orders['phonenumber']) . "</small>\n" . 
						"		</td>\n" . 
						"		<td>\n" . 
						"			<div style=\"width: 140px;white-space: nowrap;overflow-x: hidden\"><small>" . ($row_orders['payed'] == "" && $row_orders['shipping'] == "" ? "noch keine" : ($row_orders['payed'] == 1 && $row_orders['shipping'] == 1 ? "bezahlt und versendet" : ($row_orders['payed'] == 1 && $row_orders['shipping'] == 0 ? "bezahlt" : "nicht bezahlt"))) . "</small></div>\n" . 
						"		</td>\n" . 
						"		<td align=\"center\">\n" . 
...
$navigation = new navigation($conn, 3, ($parameter['link'] == "neue-auftraege" ? "order_orders" : ($parameter['link'] == "auftrag-archiv" ? "order_archive" : ($parameter['link'] == "neuer-auftrag" ? "order_new" : ""))));
...
if(!isset($_POST['add']) && !isset($_POST['edit']) && !isset($_POST['save']) && !isset($_POST['update'])){
	include("includes/codes/codes_backend_status_order.php");
}
...
$html = "<div class=\"row\">\n" . 
		...
		"<div class=\"row\">\n" . 
		"	<div class=\"col-7\">\n" . 
		"		" . (!isset($_POST['delete']) && isset($_POST['id']) && intval($_POST['id']) > 0 ? "<h3>Auftragsarchiv-Auftragsnummer: " . $row_order['order_number'] . " <i class=\"" . $payings->getPoint() . "\"></i></h3><p id=\"user_info\" class=\"mb-0\">" . ($row_order['companyname'] != "" ? "Firma: " . $row_order['companyname'] . ", " : "") . "Name: " . ($row_order['gender'] == 1 ? "Frau" : "Herr") . " " . $row_order['firstname'] . " " . $row_order['lastname'] . "</p>" : (isset($_POST['add']) && $_POST['add'] == "hinzufügen" ? "<h3>Archiv</h3>" : "<h3>Auftragsarchiv</h3>")) . "\n" . 
		"	</div>\n" . 
		...
		(
			...
			"			<button type=\"submit\" name=\"move\" value=\"Zu Aufträge\" class=\"btn btn-danger\" onclick=\"return confirm('Wollen Sie den Eintrag wircklich nach Aufträge verschieben?')\">zu Aufträge <i class=\"fa fa-list\" aria-hidden=\"true\"></i></button>\n" . 
			"			<button type=\"submit\" name=\"move_interested\" value=\"Zu Interessenten\" class=\"btn btn-danger\" onclick=\"return confirm('Wollen Sie den Eintrag wircklich nach Interessenten verschieben?')\">zu Interessenten <i class=\"fa fa-table\" aria-hidden=\"true\"></i></button>\n" . 
			"		</form>\n" : 
...
if(!isset($_POST['add']) && !isset($_POST['edit']) && !isset($_POST['save']) && !isset($_POST['update'])){
	...
	include("includes/condition/admin_order_not_add_not_edit_not_save_not_update.php");

...
$parameter['tab'] = "edit";
$parameter['link'] = "einkaeufe-archiv";

$shopping_session = "shopping";
$shopping_action = "/crm/einkaeufe-archiv";
...
$shopping_right = "shopping_archive";
$shopping_name = "Archiv-Einkauf";
...
if(isset($_POST['move']) && $_POST['move'] == "Zu Einkäufe"){

	$parameter['archive_to_shoppings_status'] = "archive_to_shoppings_status";

	include("includes/condition/admin_shopping_move_zu_einkaeufe.php");
...
$and = $where == "" ? "WHERE `shopping_shoppings`.`mode`=1 " : " AND `shopping_shoppings`.`mode`=1 ";
	...
	if($rows > 0){
		...
		while($row_shoppings = $result->fetch_array(MYSQLI_ASSOC)){
			...
			$list .= 	"<form action=\"" . $shopping_action . "\" method=\"post\">\n" . 
						"	<tr" . (isset($_POST['id']) && $_POST['id'] == $row_shoppings['id'] ? " class=\"bg-primary text-white shoppings_archiv_menu\"" : " class=\"shoppings_archiv_menu\"") . " onclick=\"if(\$(this).hasClass('active')){\$(this).removeClass('active');}else{\$(this).addClass('active');}$('#order_list_" . $row_shoppings['id'] . "').prop('checked', !$('#order_list_" . $row_shoppings['id'] . "').prop('checked'))\" data-id=\"" . $row_shoppings['id'] . "\" data-order_number=\"" . $row_shoppings['shopping_number'] . "\">\n" . 
						"		<td scope=\"row\">\n" . 
...
$html = "<div class=\"row\">\n" . 
		...
		"<div class=\"row\">\n" . 
		"	<div class=\"col-7\">\n" . 
		"		" . (!isset($_POST['delete']) && isset($_POST['id']) && intval($_POST['id']) > 0 ? "<h3>Einkäufearchiv-Bestellnummer: " . $row_shopping['shopping_number'] . "</h3>" : (isset($_POST['add']) && $_POST['add'] == "hinzufügen" ? "<h3>Archiv</h3>" : "<h3>Einkäufearchiv</h3>")) . "\n" . 
		"	</div>\n" . 
		...
		(
			...
			"			<button type=\"submit\" name=\"move\" value=\"Zu Einkäufe\" class=\"btn btn-danger\" onclick=\"return confirm('Wollen Sie den Eintrag wircklich nach Einkäufe verschieben?')\">zu Einkäufe <i class=\"fa fa-list\" aria-hidden=\"true\"></i></button>\n" . 

...
$parameter['tab'] = "edit";
$parameter['link'] = "neue-kunden";

$users_session = "users";
$users_action = "/crm/neue-kunden";
...
if(isset($param['add']) && $param['add'] == "neuen-kunde"){
	$_POST['add'] = "hinzufügen";
}

$amount_rows = isset($_SESSION[$users_session]["rows"]) && $_SESSION[$users_session]["rows"] > 0 ? $_SESSION[$users_session]["rows"] : 500;
if(!isset($param['pos'])){
	$pos = 0;
}else{
...
if(isset($_POST['save']) && $_POST['save'] == "speichern"){
	...
	include("includes/condition/admin_users_save_speichern.php");

}

if(isset($_POST['id']) && $_POST['id'] > 0){

	$row_user = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `user_users` WHERE `user_users`.`id`='" . intval($_POST['id']) . "'"), MYSQLI_ASSOC);

}

if(isset($_POST['add']) && $_POST['add'] == "hinzufügen"){

	$parameter['link'] = "neuen-kunde";
...
if(isset($_POST['move']) && $_POST['move'] == "Archiv"){
	...
	include("includes/condition/admin_users_move_archiv.php");
...
if(isset($_POST['add']) && $_POST['add'] == "hinzufügen"){
	...
	$parameter['link'] = "neuer-kunde";

	include("includes/condition/admin_users_add_hinzufuegen.php");

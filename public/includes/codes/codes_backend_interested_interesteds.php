...
$order_action = "/crm/neue-interessenten";
...
$order_right = "interested_interesteds";
...
$sorting_field_name = isset($_SESSION[$order_session]["sorting_field"]) ? $sorting[$_SESSION[$order_session]["sorting_field"]]["value"] : $sorting[4]["value"];
$sorting_field_value = isset($_SESSION[$order_session]["sorting_field"]) ? $_SESSION[$order_session]["sorting_field"] : 4;
...
if(isset($param['add']) && $param['add'] == "neuen-interessent"){
...
if(isset($_POST['multi_status']) && $_POST['multi_status'] == "durchführen" && isset($_POST['status']) && intval($_POST['status']) > 0 && isset($_POST['to_archiv'])){
	...
	$parameter['order_to_archive_status'] = "interested_to_archive_status";
...
if(isset($_POST['multi_to_archiv']) && $_POST['multi_to_archiv'] == "durchführen"){
	...
	include("includes/condition/admin_interested_multi_to_archiv.php");
...
if(isset($_POST['save']) && $_POST['save'] == "speichern"){
	...
	$parameter['tab'] = "edit";
	$parameter['link'] = "neuer-interessent";

	$parameter['link_edit'] = "neue-interessenten";

	$parameter['order_status_intern'] = "interested_status_intern";
...
if(isset($_POST['add']) && $_POST['add'] == "hinzufügen"){
	...
	$parameter['link'] = "neuen-interessent";
...
if(isset($_POST['move']) && $_POST['move'] == "Archiv"){
	...
	include("includes/condition/admin_interested_move_archiv.php");
	...
	if($rows > 0){
		...
		while($row_orders = $result->fetch_array(MYSQLI_ASSOC)){
			...
			$list .= 	"<form action=\"" . $order_action . "\" method=\"post\">\n" . 
						"	<tr" . (isset($_POST['id']) && $_POST['id'] == $row_orders['id'] ? " class=\"bg-primary text-white interesteds_menu\"" : " class=\"interesteds_menu\"") . " onclick=\"if(\$(this).hasClass('active')){\$(this).removeClass('active');}else{\$(this).addClass('active');}$('#order_list_" . $row_orders['id'] . "').prop('checked', !$('#order_list_" . $row_orders['id'] . "').prop('checked'))\" data-id=\"" . $row_orders['id'] . "\" data-order_number=\"" . $row_orders['order_number'] . "\">\n" . 
						"		<td scope=\"row\">\n" . 
...
$html = "<div class=\"row\">\n" . 
		...
		"<div class=\"row\">\n" . 
		"	<div class=\"col-7\">\n" . 
		"		" . (!isset($_POST['delete']) && isset($_POST['id']) && intval($_POST['id']) > 0 ? "<h3>Interessentenübersicht-Auftragsnummer: " . $row_order['order_number'] . "</h3><p id=\"user_info\" class=\"mb-0\">" . ($row_order['companyname'] != "" ? "Firma: " . $row_order['companyname'] . ", " : "") . "Name: " . ($row_order['gender'] == 1 ? "Frau" : "Herr") . " " . $row_order['firstname'] . " " . $row_order['lastname'] . "</p>" : (isset($_POST['add']) && $_POST['add'] == "hinzufügen" ? "<h3>Neuer Interessent</h3>" : "<h3>Interessentenübersicht</h3>")) . "\n" . 
		"	</div>\n" . 

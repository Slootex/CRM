...
$parameter['tab'] = "edit";
$parameter['link'] = "neue-einkaeufe";

$shopping_session = "shopping";
$shopping_action = "/crm/neue-einkaeufe";
$shopping_table = "shopping_shoppings";
$shopping_id_field = "shopping_id";
$shopping_mode = 0;
$shopping_archiv_mode = 1;
$shopping_hash = "";
$shopping_templates_type = 5;
$shopping_right = "shopping_shoppings";
$shopping_name = "Einkauf";
...
if(isset($param['add']) && $param['add'] == "neuer-einkauf"){
...
if(isset($_POST['multi_status']) && $_POST['multi_status'] == "durchführen" && isset($_POST['status']) && intval($_POST['status']) > 0 && !isset($_POST['to_archiv'])){
	...
	include("includes/condition/admin_shopping_multi_status.php");

}

if(isset($_POST['multi_status']) && $_POST['multi_status'] == "durchführen" && isset($_POST['status']) && intval($_POST['status']) > 0 && isset($_POST['to_archiv'])){

	$parameter['shopping_to_archive_status'] = "shopping_to_archive_status";

	include("includes/condition/admin_shopping_multi_status_to_archiv.php");
...
if(isset($_POST['multi_to_archiv']) && $_POST['multi_to_archiv'] == "durchführen"){
	...
	include("includes/condition/admin_shopping_multi_to_archiv.php");
...
if(isset($_POST['save']) && $_POST['save'] == "speichern"){
	...
	$parameter['link_edit'] = "neue-einkaeufe";

	$parameter['shopping_status'] = "shopping_status";

	include("includes/condition/admin_shopping_save_speichern.php");
...
if(isset($_POST['move']) && $_POST['move'] == "Archiv"){
	...
	include("includes/condition/admin_shopping_move_archiv.php");

}

if(isset($_POST['move_retoures']) && $_POST['move_retoures'] == "Zu Retouren"){

	$parameter['link'] = "neue-einkaeufe";

	$parameter['shopping_to_retoures_status'] = "shopping_to_retoures_status";

	include("includes/condition/admin_shopping_move_retoures_zu_retouren.php");
	...
	if($rows > 0){
		...
		while($row_shoppings = $result->fetch_array(MYSQLI_ASSOC)){
			...
			$list .= 	"<form action=\"" . $shopping_action . "\" method=\"post\">\n" . 
						"	<tr" . (isset($_POST['id']) && $_POST['id'] == $row_shoppings['id'] ? " class=\"bg-primary text-white shoppings_menu\"" : " class=\"shoppings_menu\"") . " onclick=\"if(\$(this).hasClass('active')){\$(this).removeClass('active');}else{\$(this).addClass('active');}$('#order_list_" . $row_shoppings['id'] . "').prop('checked', !$('#order_list_" . $row_shoppings['id'] . "').prop('checked'))\" data-id=\"" . $row_shoppings['id'] . "\" data-order_number=\"" . $row_shoppings['shopping_number'] . "\">\n" . 
						"		<td scope=\"row\">\n" . 
						...
						"		</td>\n" . 
						"		<td align=\"center\">\n" . 
						"			<span class=\"badge badge-pill\" style=\"background-color: " . $row_shoppings['status_color'] . "\">" . $row_shoppings['status_name'] . "<sup>" . $row_shoppings['status_counter'] . "</sup></span>\n" . 
						"		</td>\n" . 
						"		<td class=\"text-nowrap\">\n" . 
		...
		$list = isset($_POST['search']) && $_POST['search'] == "suchen" ? "<script>alert('Es wurden keine mit deiner Suchanfrage - " . $_SESSION[$shopping_session]["keyword"] . " - übereinstimmende Einkäufe gefunden.')</script>\n" : "";

	}

}

$result_statuses = mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`type`='5' AND `statuses`.`extra_search`='1' ORDER BY CAST(`statuses`.`id` AS UNSIGNED) ASC");

$extra_search_options = "						<option value=\"0\"" . (isset($_SESSION[$shopping_session]['extra_search']) && $_SESSION[$shopping_session]['extra_search'] == 0 ? " selected=\"selected\"" : "") . ">Alle Vorgänge</option>\n";
...
$navigation = new navigation($conn, 6, ($parameter['link'] == "neue-einkaeufe" ? "shopping_shoppings" : ($parameter['link'] == "einkaeufe-archiv" ? "shopping_archive" : ($parameter['link'] == "neuer-einkauf" ? "shopping_new" : ""))));
...
$html = "<div class=\"row\">\n" . 
		...
		"				<div class=\"col-sm-12\">\n" . 
		"					<div class=\"btn-group\">\n" . 
		"						<input type=\"text\" id=\"global_keyword\" name=\"global_keyword\" value=\"" . (isset($_SESSION['global_shoppings']['keyword']) && $_SESSION['global_shoppings']['keyword'] != "" ? $_SESSION['global_shoppings']['keyword'] : "") . "\" placeholder=\"Suchbegriff / Barcode\" aria-label=\"Suchbegriff / Barcode\" class=\"form-control bg-" . $_SESSION["admin"]["bgcolor_select"] . " text-" . $_SESSION["admin"]["color_select"] . " border border-" . $_SESSION["admin"]["border_select"] . "\" style=\"border-radius: .25rem 0 0 .25rem\" />\n" . 
		...
		"<div class=\"row\">\n" . 
		"	<div class=\"col-7\">\n" . 
		"		" . (!isset($_POST['delete']) && isset($_POST['id']) && intval($_POST['id']) > 0 ? "<h3>Einkäufeübersicht-Bestellnummer: " . $row_shopping['shopping_number'] . "</h3>" : (isset($_POST['add']) && $_POST['add'] == "hinzufügen" ? "<h3>Neuer Einkauf</h3>" : "<h3>Einkäufeübersicht</h3>")) . "\n" . 
		"	</div>\n" . 
		...
		(
			...
			"			<button type=\"submit\" name=\"move_retoures\" value=\"Zu Retouren\" class=\"btn btn-danger\" onclick=\"return confirm('Wollen Sie den Eintrag wircklich nach Retouren verschieben?')\">zu Retouren <i class=\"fa fa-table\" aria-hidden=\"true\"></i></button>\n" . 
			"		</form>\n" : 
...
if(!isset($_POST['add']) && !isset($_POST['edit']) && !isset($_POST['save']) && !isset($_POST['update'])){
	...
	include("includes/condition/admin_shopping_not_add_not_edit_not_save_not_update.php");

}

if(isset($_POST['add']) && $_POST['add'] == "hinzufügen"){

	include("includes/condition/admin_shopping_add_hinzufuegen.php");

}

if(isset($_POST['edit']) && $_POST['edit'] == "bearbeiten"){

	include("includes/condition/admin_shopping_edit_bearbeiten.php");

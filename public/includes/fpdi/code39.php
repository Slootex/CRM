...
class PDF_Code39 extends \FPDF {
	...
	function Code39($xpos, $ypos, $code, $baseline=0.5, $height=5, $text=0){
	...
	function Code39_90_degree($xpos, $ypos, $code, $baseline=0.5, $width=5, $text=0){
		...
		$this->SetFont('Arial','',10);
		if($text == 1){
		  $this->Text($xpos + $width + 4, $ypos, $code);
		}
		$this->SetFillColor(0);
		...
		for($i=0; $i<strlen($code); $i++){
			...
			for($bar=0; $bar<9; $bar++){
				...
				if($bar % 2 == 0){
					$this->Rect($xpos, $ypos - $lineHeight, $width, $lineHeight, 'F');
				}
				$ypos -= $lineHeight;
			}
			$ypos -= $gap;
	...
	function Code128_90_degree($x, $y, $code, $w, $h) {
		$Aguid = "";                                                                      // Création des guides de choix ABC
		$Bguid = "";
		$Cguid = "";
		for ($i=0; $i < strlen($code); $i++) {
		...
		for ($i=0; $i<strlen($crypt); $i++) {                                      // BOUCLE D'IMPRESSION
			$c = $this->T128[ord($crypt[$i])];
			for ($j=0; $j<count($c); $j++) {
				$this->Rect($x,$y-($c[$j]*$modul),$w,$c[$j]*$modul,"F");
				$y -= ($c[$j++]+$c[$j])*$modul;
			}
		}
	}

		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12 col-12">
				<nav class="navbar navbar-expand-lg navbar-<?php echo $_SESSION["admin"]["bgcolor_header_footer"]; ?> p-0">
					...
					<?php 
						$navigation = new navigation($conn, 2, "pickup_pickup");
					...
					<div id="navbar_1" class="navbar-collapse collapse justify-content-end">
						...
						<ul id="menu-1" class="navbar-nav mr-0">
							<?php 
								$navigation = new navigation($conn, 1, "pickup_pickup");
								echo $navigation->show();
							?>
...
function initAutocomplete() {
	...
	autocomplete_route = new google.maps.places.Autocomplete(document.getElementById('pickup_street'), {types: ['geocode']});
	autocomplete_route.setFields(['address_component']);
	autocomplete_route.addListener('place_changed', fillInAddress);

	autocomplete_postal_code = new google.maps.places.Autocomplete(document.getElementById('pickup_zipcode'), {types: ['(regions)']});
	autocomplete_postal_code.setFields(['address_component']);
	autocomplete_postal_code.addListener('place_changed', fillInPostalCode);

	autocomplete_locality = new google.maps.places.Autocomplete(document.getElementById('pickup_city'), {types: ['(cities)']});
...
function fillInAddress() {
  ...
  for (var component in componentForm) {
 	  if(component == "country"){
		...
		document.getElementById('pickup_street').disabled = false;
      }else if(component == "street_number"){
        document.getElementById('pickup_streetno').value = '';
		document.getElementById('pickup_streetno').disabled = false;
      }else if(component == "postal_code"){
        document.getElementById('pickup_zipcode').value = '';
		document.getElementById('pickup_zipcode').disabled = false;
  ...
  for (var i = 0; i < place.address_components.length; i++) {
    ...
    if (componentForm[addressType]) {
	  ...
	  if(addressType == "country"){
		...
		$('#pickup_country option[value=\'' + countryToId[val] + '\']').attr('selected', 'selected');
		setSelect('#pickup_country');
	  }else if(addressType == "route"){
        document.getElementById('pickup_street').value = val;
      }else if(addressType == "street_number"){
        document.getElementById('pickup_streetno').value = val;
      }else if(addressType == "postal_code"){
        document.getElementById('pickup_zipcode').value = val;

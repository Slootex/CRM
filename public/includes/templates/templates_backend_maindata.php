...
<body>
<div id="header" class="bg-<?php echo $_SESSION["admin"]["bgcolor_header_footer"]; ?> border-bottom border-<?php echo $_SESSION["admin"]["order_header_footer"]; ?> mb-3">
		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12 col-12">
				<nav class="navbar navbar-expand-lg navbar-<?php echo $_SESSION["admin"]["bgcolor_header_footer"]; ?> p-0">
					...
					<?php 
						$navigation = new navigation($conn, 2, "maindata");
					...
					<div id="navbar_1" class="navbar-collapse collapse justify-content-end">
						...
						<ul id="menu-1" class="navbar-nav mr-0">
							<?php 
								$navigation = new navigation($conn, 1, "maindata");
								echo $navigation->show();
							?>
...
function initAutocomplete() {
	...
	autocomplete_route = new google.maps.places.Autocomplete(document.getElementById('street'), {types: ['geocode']});
	autocomplete_route.setFields(['address_component']);
	autocomplete_route.addListener('place_changed', fillInAddress);

	autocomplete_postal_code = new google.maps.places.Autocomplete(document.getElementById('zipcode'), {types: ['(regions)']});
	autocomplete_postal_code.setFields(['address_component']);
	autocomplete_postal_code.addListener('place_changed', fillInPostalCode);

	autocomplete_locality = new google.maps.places.Autocomplete(document.getElementById('city'), {types: ['(cities)']});
  ...
  for (var i = 0; i < place.address_components.length; i++) {
    ...
    if (componentForm[addressType]) {
	  ...
	  if(addressType == "country"){
		...
		$('#country option[value=\'' + countryToId[val] + '\']').attr('selected', 'selected');
		setSelect('#country');
	  }else if(addressType == "route"){
        document.getElementById('street').value = val;
      }else if(addressType == "street_number"){
        document.getElementById('streetno').value = val;

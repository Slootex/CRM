				...
				while($row_packing = $result_packings->fetch_array(MYSQLI_ASSOC)){
					...
					mysqli_query($conn, "	INSERT 	`order_orders_shipments` 
											SET 	`order_orders_shipments`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
													...
													`order_orders_shipments`.`total_charges_with_taxes`='" . mysqli_real_escape_string($conn, number_format(($row_packing1['amount'] + $saturday_costs[$radio_saturday] + $carriers_services_costs[$row_packing1['carriers_service']] + $shipping_costs[$row_last_paying['radio_shipping']] + $payment_costs[$row_last_paying['radio_payment']]), 2, '.', '')) . "', 
					...
					mysqli_query($conn, "	INSERT 	`" . $order_table . "_events` 
											SET 	`" . $order_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
													...
													`" . $order_table . "_events`.`message`='" . mysqli_real_escape_string($conn, $order_name . ", Sendung (Multi-Extern) durchgeführt, ID [#" . $_SESSION["shipments"]["id"]) . "]', 
					...
					mysqli_query($conn, "	INSERT 	`" . $order_table . "_history` 
											SET 	`" . $order_table . "_history`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
													...
													`" . $order_table . "_history`.`message`='" . mysqli_real_escape_string($conn, $row_status['name'] . " - Die (Multi) Sendung " . strip_tags($response->ShipmentResponse->ShipmentResults->ShipmentIdentificationNumber) . " wurde über Extern versendet.") . "', 
...
//	$_POST['id'] = 0;
	...
	$_POST['pack_adding_extern_shipping'] = "bearbeiten";

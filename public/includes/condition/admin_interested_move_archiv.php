<?php 
	...
	mysqli_query($conn, "	UPDATE	`order_orders` 
							SET 	`order_orders`.`admin_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["id"])) . "', 
									...
									`order_orders`.`mode`='" . mysqli_real_escape_string($conn, $order_archiv_mode) . "', 
									`order_orders`.`loc_date`='" . mysqli_real_escape_string($conn, ($order_archiv_mode == 1 ? $time : "")) . "', 
	...
	mysqli_query($conn, "INSERT 	`" . $order_table . "_events` 
							SET 	`" . $order_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`" . $order_table . "_events`.`message`='" . mysqli_real_escape_string($conn, "Interessent nach Archiv verschoben, ID [#" . $_SESSION["status"]["id"] . "]") . "', 

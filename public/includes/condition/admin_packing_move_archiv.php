<?php 
	...
	mysqli_query($conn, "	UPDATE	`packing_packings` 
							SET 	`packing_packings`.`admin_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["id"])) . "', 
									`packing_packings`.`mode`='" . mysqli_real_escape_string($conn, $packing_archiv_mode) . "', 
	...
	$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($maindata[$parameter['packing_to_archive_status']])) . "'"), MYSQLI_ASSOC);
	...
	for($j = 0;$j < count($fields);$j++){
		...
		$row_template[$fields[$j]] = str_replace("[id]", $row_packing['packing_number'], $row_template[$fields[$j]]);
	...
	mysqli_query($conn, "	INSERT 	`" . $packing_table . "_events` 
							SET 	`" . $packing_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`" . $packing_table . "_events`.`message`='" . mysqli_real_escape_string($conn, "Packtisch nach Archiv verschoben, ID [#" . $_SESSION["status"]["id"] . "]") . "', 
	...
	if(!isset($param['archiv'])){
		//header('Location: /crm/einkaeufe-archiv/bearbeiten/' . intval($_POST['id']));
	}

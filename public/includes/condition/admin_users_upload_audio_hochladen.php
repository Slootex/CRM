<?php 
			...
			foreach($_FILES["file"]["error"] as $key => $error) {
				//if ($j <= 5 && $_FILES["file"]["name"][$key] != "") {
					...
					if(in_array($ext, $allowed)){
						...
						move_uploaded_file($_FILES["file"]["tmp_name"][$key], $uploaddir . "company/" . intval($_SESSION["admin"]["company_id"]) . "/user/" . $row_user['user_number'] . "/audio/" . basename($random . '_' . $_FILES["file"]["name"][$key]));
	...
	if($emsg_audio == "" && $files != ""){
		...
		mysqli_query($conn, "	UPDATE 	`user_users` 
								SET 	`user_users`.`audio`='" . mysqli_real_escape_string($conn, ($row_user['audio'] == "" ? $files : $row_user['audio'] . "\r\n" . $files)) . "' 
		...
		mysqli_query($conn, "	INSERT 	`user_users_events` 
								SET 	`user_users_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										`user_users_events``='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["id"])) . "', 
		...
		$emsg_audio = "<p>Der " . $users_name . " wurde erfolgreich geändert!</p>\n";

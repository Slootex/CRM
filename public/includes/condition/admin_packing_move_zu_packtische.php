<?php 
	...
	$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($maindata[$parameter['archive_to_packings_status']])) . "'"), MYSQLI_ASSOC);
	...
	mysqli_query($conn, "	INSERT 	`" . $packing_table . "_events` 
							SET 	`" . $packing_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`" . $packing_table . "_events`.`message`='" . mysqli_real_escape_string($conn, "Archiv nach Packtische verschoben, ID [#" . $_SESSION["status"]["id"] . "]") . "', 
	...
	header('Location: /crm/neue-packtische/bearbeiten/' . intval($_POST['id']));

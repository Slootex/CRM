<?php 
	...
	$result = mysqli_query($conn, "	SELECT 		`" . $shopping_table . "_events`.`id` AS id, 
												(
													SELECT 	name 
													FROM 	`admin` `admin` 
													WHERE 	`admin`.`id`=`" . $shopping_table . "_events`.`admin_id` 
													AND 	`admin`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' 
												) AS admin_name, 
												`" . $shopping_table . "_events`.`type` AS type, 
												`" . $shopping_table . "_events`.`message` AS message, 
												`" . $shopping_table . "_events`.`subject` AS subject, 
												`" . $shopping_table . "_events`.`body` AS body, 
												`" . $shopping_table . "_events`.`time` AS time 
									FROM 		`" . $shopping_table . "_events` `" . $shopping_table . "_events` 
									WHERE 		`" . $shopping_table . "_events`.`shopping_id`='" . mysqli_real_escape_string($conn, intval($row_shopping['id'])) . "' 
									AND 		`" . $shopping_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' 
									ORDER BY 	CAST(`" . $shopping_table . "_events`.`time` AS UNSIGNED) DESC");
	...
	$tabs_contents .= 	"				<div class=\"row\">\n" . 
						...
						"										<form action=\"" . $shopping_action . "\" method=\"post\" style=\"float: right\" class=\"\">\n" . 
						"											<input type=\"hidden\" name=\"id\" value=\"" . intval($_POST['id']) . "\" />\n" . 
						"											<button type=\"submit\" name=\"delete\" value=\"Retoure unwiederuflich löschen\" onclick=\"return confirm('Wollen Sie den Eintrag wircklich entfernen?')\" class=\"btn btn-danger\">Retoure unwiederuflich löschen <i class=\"fa fa-ban\" aria-hidden=\"true\"></i></button>\n" . 
						"										</form>\n" . 
						"									</div>\n" . 
						"								</div>\n" . 
						"							</div>\n" . 

<?php 
	...
	if($emsg == ""){
		...
		for($i = 0;$i < count($ids);$i++){
			...
			$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($_POST['status'])) . "'"), MYSQLI_ASSOC);
			...
			mysqli_query($conn, "	UPDATE	`shopping_shoppings` 
									...
									WHERE 	`shopping_shoppings`.`id`='" . mysqli_real_escape_string($conn, intval($ids[$i])) . "' 
	...
	//				$row_template[$fields[$j]] = str_replace("[recall]", ($row_order['recall_date'] != "" ? date("d.m.Y", $row_order['recall_date']) . " - " . $recall_time : ""), $row_template[$fields[$j]]);
			...
			mysqli_query($conn, "	INSERT 	`" . $shopping_table . "_statuses` 
									SET 	`" . $shopping_table . "_statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
											`" . $shopping_table . "_statuses`.`shopping_id`='" . mysqli_real_escape_string($conn, intval($ids[$i])) . "', 
			...
			mysqli_query($conn, "	INSERT 	`" . $shopping_table . "_events` 
									SET 	`" . $shopping_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
											...
											`" . $shopping_table . "_events`.`shopping_id`='" . mysqli_real_escape_string($conn, intval($ids[$i])) . "', 
											`" . $shopping_table . "_events`.`type`='" . mysqli_real_escape_string($conn, 0) . "', 
											`" . $shopping_table . "_events`.`message`='" . mysqli_real_escape_string($conn, "Verlauf, " . $row_status['name'] . ", ID [#" . $_SESSION["status"]["id"] . "]") . "', 
			...
			if($row_template['mail_with_pdf'] == 1){
				...
				if(isset($row_order['id']) && $row_status['usermail'] == 1 && $row_order['email'] != "" && !isset($_POST['no_email'])){

	...
	if($emsg == ""){
		...
		mysqli_query($conn, "	INSERT 	`packing_packings` 
								SET 	`packing_packings`.`mode`='0', 
										...
										`packing_packings`.`address_id`='0', 
										`packing_packings`.`is_send`='0', 
										`packing_packings`.`shipments_id`='', 
										`packing_packings`.`files`='', 
										...
										`packing_packings`.`gender`='" . mysqli_real_escape_string($conn, intval($_POST['gender'])) . "', 
										...
										`packing_packings`.`radio_payment`='" . mysqli_real_escape_string($conn, (isset($_POST['radio_payment']) ? intval($_POST['radio_payment']) : 0)) . "', 
										`packing_packings`.`radio_saturday`='" . mysqli_real_escape_string($conn, (isset($_POST['radio_saturday']) ? intval($_POST['radio_saturday']) : 0)) . "', 
										`packing_packings`.`amount`='" . mysqli_real_escape_string($conn, (isset($_POST['amount']) ? str_replace(",", ".", strip_tags($_POST['amount'])) : 0)) . "', 
			...
			if(isset($_POST['device_' . $row_device['id']]) && intval($_POST['device_' . $row_device['id']]) == 1){
				...
				mysqli_query($conn, "	UPDATE 	`order_orders_devices` 
										SET 	`order_orders_devices`.`admin_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["id"])) . "', 
												`order_orders_devices`.`is_shipping_user`='1', 
		...
		for($j = 0;$j < count($fields);$j++){
			...
			$row_template[$fields[$j]] = str_replace("[id]", $packing_number, $row_template[$fields[$j]]);
...
//				$row_template[$fields[$j]] = str_replace("[files]", $links, $row_template[$fields[$j]]);
		...
		mysqli_query($conn, "	INSERT 	`packing_packings_events` 
								SET 	`packing_packings_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										...
										`packing_packings_events`.`message`='" . mysqli_real_escape_string($conn, "Warenausgang, Kunde erstellt, ID [#" . $_SESSION["status"]["id"] . "]") . "', 

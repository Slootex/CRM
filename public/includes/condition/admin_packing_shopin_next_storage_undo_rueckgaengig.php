<?php 
	...
	mysqli_query($conn, "	INSERT 	`order_orders_events` 
							SET 	`order_orders_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`order_orders_events`.`message`='" . mysqli_real_escape_string($conn, "Auftrag, Gerät " . $row_device['device_number'] . " Lagerplatz entfernt, ID [#" . intval($row_shopin['device_id']) . "]") . "', 
									`order_orders_events`.`subject`='" . mysqli_real_escape_string($conn, "Auftrag, Gerät " . $row_device['device_number'] . " Lagerplatz entfernt, ID [#" . intval($row_shopin['device_id']) . "]") . "', 
	...
	mysqli_query($conn, "	INSERT 	`order_orders_devices_events` 
							SET 	`order_orders_devices_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`order_orders_devices_events`.`message`='" . mysqli_real_escape_string($conn, "<span class=\"badge badge-danger\">Gerät " . $row_device['device_number'] . " Lagerplatz entfernt</span> <span class=\"badge badge-danger\">Lagerplatz " . $row_device['storage_space'] . " entfernt, ID [#" . $row_device['id'] . "]</span>") . "', 

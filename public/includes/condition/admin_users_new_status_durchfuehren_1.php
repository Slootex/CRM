<?php 
	...
	if($emsg == ""){
		...
		$row_user = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `user_users` WHERE `user_users`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `user_users`.`id`='" . intval($_POST['id']) . "'"), MYSQLI_ASSOC);
		...
		$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . intval($_POST['status_id']) . "'"), MYSQLI_ASSOC);
		...
		mysqli_query($conn, "	INSERT 	`user_users_events` 
								SET 	`user_users_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										...
										`user_users_events`.`message`='" . mysqli_real_escape_string($conn, $users_name . ", Verlauf, " . $row_status['name'] . ", ID [#" . $_SESSION["status"]["id"] . "]") . "', 

<?php 
	...
	switch(strip_tags($_POST['history_status_order'])){
		case "speichern_0":
			$_POST['status'] = 0;
			break;
		case "speichern_1":
	...
	if(intval($_POST['status']) == 2){
		mysqli_query($conn, "	UPDATE 	`" . $order_table . "_history` 
								SET 	`" . $order_table . "_history`.`status`='" . mysqli_real_escape_string($conn, intval($_POST['status'])) . "', 
										`" . $order_table . "_history`.`message`=CONCAT(`" . $order_table . "_history`.`message`, ' - Erledigt durch: " . mysqli_real_escape_string($conn, $row_admin['name']) . " (" . date("d.m.Y - H:i", time()) . ")') 

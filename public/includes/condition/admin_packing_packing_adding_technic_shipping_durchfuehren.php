<?php 
	...
	if($emsg == ""){
		...
		if($row_packing1['address_service'] == "65" && $row_packing1['radio_saturday'] == 1){
		...
		if($row_packing1['radio_payment'] == 1){
			$saturday_delivery['COD'] = array(
				...
				'CODAmount' => array(
					'CurrencyCode' => 'EUR', 
					'MonetaryValue' => '' . ($row_packing1['amount'] + $saturday_costs[$row_packing1['radio_saturday']] + $carriers_services_costs[$row_packing1['address_service']] + $shipping_costs[$row_packing1['radio_shipping']] + $payment_costs[$row_packing1['radio_payment']])
				)
			);
		}
		...
		$data = array(
			'ShipmentRequest' => array(
				'Shipment' => array(
					...
					'Service' => array(
						'Code' => '' . intval($row_packing1['address_service']), 
						'Description' => $carriers_services[$row_packing1['address_service']]
					), 
					'Package' => array(
						array(
							'Description' => 'International Goods', 
							'ReferenceNumber' => array(
								'Value' => $row_packing1['packing_number'] . "-" . $row_order['order_number'], 
							),
							'Packaging' => array(
								'Code' => '02'
							), 
				...
				while($row_packing = $result_packings->fetch_array(MYSQLI_ASSOC)){
					...
					mysqli_query($conn, "	INSERT 	`" . $order_table . "_events` 
											SET 	`" . $order_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
													...
													`" . $order_table . "_events`.`message`='" . mysqli_real_escape_string($conn, $order_name . ", Sendung (Multi-Techniker) durchgeführt, ID [#" . $_SESSION["shipments"]["id"]) . "]', 
					...
					mysqli_query($conn, "	INSERT 	`" . $order_table . "_history` 
											SET 	`" . $order_table . "_history`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
													...
													`" . $order_table . "_history`.`message`='" . mysqli_real_escape_string($conn, $row_status['name'] . " - Die (Multi) Sendung " . strip_tags($response->ShipmentResponse->ShipmentResults->ShipmentIdentificationNumber) . " wurde an den Techniker versendet.") . "', 

<?php 
	...
	if($emsg == ""){
		...
		$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($maindata['order_recall_status'])) . "'"), MYSQLI_ASSOC);
		...
		for($j = 0;$j < count($fields);$j++){
			...
			$row_template[$fields[$j]] = str_replace("[recall]", ($row_order['recall_date'] != "" ? date("d.m.Y - H:i", $row_order['recall_date']) : ""), $row_template[$fields[$j]]);
		...
		mysqli_query($conn, "	INSERT 	`" . $order_table . "_customer_messages` 
								SET 	`" . $order_table . "_customer_messages`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										...
										`" . $order_table . "_customer_messages`.`message`='" . mysqli_real_escape_string($conn, "Rückruf vereinbart") . "', 
		...
		mysqli_query($conn, "	INSERT 	`" . $order_table . "_events` 
								SET 	`" . $order_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										...
										`" . $order_table . "_events`.`body`='" . mysqli_real_escape_string($conn, "Rückruf vereinbart") . "', 
										`" . $order_table . "_events`.`time`='" . mysqli_real_escape_string($conn, intval($time)) . "'");

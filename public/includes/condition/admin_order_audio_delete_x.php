<?php 
	...
	$row_file = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `order_orders_audios` WHERE `order_orders_audios`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `order_orders_audios`.`id`='" . mysqli_real_escape_string($conn, intval($_POST['item'])) . "'"), MYSQLI_ASSOC);
	...
	mysqli_query($conn, "	DELETE FROM	`order_orders_audios` 
							...
							AND 		`order_orders_audios`.`id`='" . mysqli_real_escape_string($conn, intval($_POST['item'])) . "'");

	@unlink("uploads/company/" . intval($_SESSION["admin"]["company_id"]) . "/order/" . $row_order['order_number'] . "/audio/" . $row_file['file']);

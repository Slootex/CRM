<?php 
	...
	mysqli_query($conn, "UPDATE		`user_users` 
							SET 	`user_users`.`mode`='" . mysqli_real_escape_string($conn, intval($users_mode)) . "' 
	...
	mysqli_query($conn, "INSERT 	`user_users_events` 
							SET 	`user_users_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`user_users_events`.`message`='" . mysqli_real_escape_string($conn, $users_name . ", zu Kunden verschoben, ID [#" . intval($_POST["id"]) . "]") . "', 
									`user_users_events`.`subject`='" . mysqli_real_escape_string($conn, $users_name . ", zu Kunden verschoben, ID [#" . intval($_POST["id"]) . "]") . "', 

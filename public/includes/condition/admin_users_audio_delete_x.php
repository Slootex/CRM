<?php 
	...
	for($i = 0;$i < count($arr_files);$i++){
		if($i == intval($_POST['item'])){
			@unlink("uploads/company/" . intval($_SESSION["admin"]["company_id"]) . "/user/" . $row_user['user_number'] . "/audio/" . $arr_files[$i]);
			$file_id = ($i + 1);
	...
	mysqli_query($conn, "UPDATE 	`user_users` 
							SET 	`user_users`.`audio`='" . mysqli_real_escape_string($conn, $files) . "' 

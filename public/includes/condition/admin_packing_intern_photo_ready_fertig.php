<?php 
	...
	if(isset($row_order['id']) && $row_order['mode'] < 2){
		...
		$status_field = "order_photo_status";
		...
		$status_field = "interested_photo_status";
		...
		if(file_exists("uploads/company/" . intval($_SESSION["admin"]["company_id"]) . "/scan/" . $row_order['order_number'] . ".pdf")){
			...
			mysqli_query($conn, "	INSERT 	`order_orders_files` 
									SET 	`order_orders_files`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
											...
											`order_orders_files`.`device_id`='" . mysqli_real_escape_string($conn, intval($row_intern["device_id"])) . "', 
...
//				$row_template[$fields[$j]] = str_replace("[files]", $links, $row_template[$fields[$j]]);
		...
		$emsg = "<p>Das Fotografieren wurde erfolgreich durchgeführt!</p>\n";

	...
	if($emsg == ""){
		...
		mysqli_query($conn, "	INSERT 	`shopping_shoppings` 
								SET 	`shopping_shoppings`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										`shopping_shoppings`.`mode`='" . mysqli_real_escape_string($conn, intval(isset($_POST['mode']) ? $_POST['mode'] : 0)) . "', 
										`shopping_shoppings`.`creator_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["id"])) . "', 
		...
		$_POST["shopping_id"] = $conn->insert_id;

		if(isset($_FILES['file_image']['name'])){
			move_uploaded_file($_FILES["file_image"]["tmp_name"], "uploads/company/" . intval($_SESSION["admin"]["company_id"]) . "/shopping/url_" . intval($_POST['shopping_id']) . ".pdf");
		}
		...
		$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($maindata[(isset($_POST['mode']) && intval($_POST['mode']) < 2 ? 'shopping_status' : 'retoure_status')])) . "'"), MYSQLI_ASSOC);
...
//				$row_template[$fields[$j]] = str_replace("[files]", $links, $row_template[$fields[$j]]);
		...
		$shopping_table = isset($_POST['mode']) && intval($_POST['mode']) < 2 ? 'shopping_shoppings' : 'shopping_retoures';
		...
		mysqli_query($conn, "	INSERT 	`" . $shopping_table . "_statuses` 
								SET 	`" . $shopping_table . "_statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										`" . $shopping_table . "_statuses`.`shopping_id`='" . mysqli_real_escape_string($conn, intval($_POST["shopping_id"])) . "', 
										`" . $shopping_table . "_statuses`.`status_number`='" . mysqli_real_escape_string($conn, $status_number) . "', 
										`" . $shopping_table . "_statuses`.`admin_id`='" . mysqli_real_escape_string($conn, $_SESSION["admin"]["id"]) . "', 
		...
		mysqli_query($conn, "	INSERT 	`" . $shopping_table . "_events` 
								SET 	`" . $shopping_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										...
										`" . $shopping_table . "_events`.`shopping_id`='" . mysqli_real_escape_string($conn, intval($_POST["shopping_id"])) . "', 

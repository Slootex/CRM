<?php 
	...
	$row_status_admin_id = mysqli_fetch_array(mysqli_query($conn, "	SELECT 		`shopping_shoppings_statuses`.`admin_id` AS admin_id 
																	...
																	ORDER BY 	CAST(`shopping_shoppings_statuses`.`time` AS UNSIGNED) DESC limit 0, 1"), MYSQLI_ASSOC);
	...
	mysqli_query($conn, "	UPDATE	`shopping_shoppings` 
							SET 	`shopping_shoppings`.`admin_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["id"])) . "', 
									...
									`shopping_shoppings`.`mode`='0', 
									`shopping_shoppings`.`hash`='', 
	...
	$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($maindata[$retoure_to_shoppings_status])) . "'"), MYSQLI_ASSOC);
	...
	mysqli_query($conn, "	INSERT 	`" . $shopping_table . "_events` 
							SET 	`" . $shopping_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`" . $shopping_table . "_events`.`message`='" . mysqli_real_escape_string($conn, "Retoure nach Einkäufe verschoben, ID [#" . $_SESSION["status"]["id"] . "]") . "', 
	...
	header("Location: /crm/neue-einkaeufe/bearbeiten/" . intval($_POST["id"]));
	exit();

					...
					if(in_array($ext, $allowed)){
						...
						mysqli_query($conn, "	INSERT 	`order_orders_files` 
												SET 	`order_orders_files`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
														...
														`order_orders_files`.`order_id`='" . mysqli_real_escape_string($conn, intval($_POST['order_id'])) . "', 
	...
	if($emsg_files == "" && $files != ""){
		...
		mysqli_query($conn, "	INSERT 	`" . $order_table . "_events` 
								SET 	`" . $order_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										...
										`" . $order_table . "_events`.`message`='" . mysqli_real_escape_string($conn, $order_name . ", Dateien hochgeladen, ID [#" . intval($_POST["order_id"]) . "]") . "', 
										`" . $order_table . "_events`.`subject`='" . mysqli_real_escape_string($conn, $order_name . ", Dateien hochgeladen, ID [#" . intval($_POST["order_id"]) . "]") . "', 

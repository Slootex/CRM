	...
	if($emsg == ""){
		...
		mysqli_query($conn, "	INSERT 	`order_orders` 
								SET 	`order_orders`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
										`order_orders`.`mode`='" . mysqli_real_escape_string($conn, intval($_POST['order_mode'])) . "', 
										...
										`order_orders`.`transfer_date`='" . mysqli_real_escape_string($conn, intval($_POST['order_mode'] < 2 ? $time : 0)) . "', 
		...
		$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . $maindata['order_status_intern'] . "'"), MYSQLI_ASSOC);
		...
		$_POST["shopin_order_add"] = "bearbeiten";

	}else{

		$_POST["shopin_order_add"] = "hinzufuegen";

<?php 
	...
	$row_status = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `statuses` WHERE `statuses`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "' AND `statuses`.`id`='" . mysqli_real_escape_string($conn, intval($maindata[$parameter['archive_to_shoppings_status']])) . "'"), MYSQLI_ASSOC);
	...
	mysqli_query($conn, "	INSERT 	`" . $shopping_table . "_events` 
							SET 	`" . $shopping_table . "_events`.`company_id`='" . mysqli_real_escape_string($conn, intval($_SESSION["admin"]["company_id"])) . "', 
									...
									`" . $shopping_table . "_events`.`message`='" . mysqli_real_escape_string($conn, "Archiv nach Einkäufe verschoben, ID [#" . $_SESSION["status"]["id"] . "]") . "', 

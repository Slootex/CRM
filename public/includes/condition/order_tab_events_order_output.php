<?php 
	...
	$result = mysqli_query($conn, "	SELECT 		`" . $order_table . "_events`.`id` AS id, 
									...
									WHERE 		`" . $order_table . "_events`.`" . $order_id_field . "`='" . mysqli_real_escape_string($conn, intval($row_order['id'])) . "' 
	...
	$tabs_contents .= 	"				<div class=\"row\">\n" . 
						...
						($row_order['devices_amount'] == 0 && $row_order['packings_amount'] == 0 && $row_order['interns_amount'] == 0 && $row_order['shopins_amount'] == 0 ?
							"											<button type=\"submit\" name=\"delete\" value=\"Auftrag unwiederuflich löschen\" onclick=\"return confirm('Wollen Sie den Eintrag wircklich entfernen?')\" class=\"btn btn-danger\">Auftrag unwiederuflich löschen <i class=\"fa fa-ban\" aria-hidden=\"true\"></i></button>\n"
						: 
